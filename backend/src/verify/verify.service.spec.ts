...
describe('VerifyService', () => {
  ...
  describe('USE', () => {
    it('정상 — Usage row 생성 + score 50 반환', async () => {
      ...
      const out = await service.verifyAndRecord('1', UsageKind.USE, mockImage(), {})
    ...
    it('meta(gameId/lat/lng) 모두 적재', async () => {
      ...
      prisma.usage.create.mockResolvedValue({
        id: 1n, kind: UsageKind.USE, score: 50, scannedAt: new Date(),
      })

      await service.verifyAndRecord('1', UsageKind.USE, mockImage(), {
        gameId: '42', lat: 37.512, lng: 127.071,
      })
  ...
  describe('RETURN', () => {
    ...
    it('정상 — 12시간 내 USE 있음 → score 100', async () => {
      ...
      const out = await service.verifyAndRecord('1', UsageKind.RETURN, mockImage(), {})
    ...
    it('findFirst가 cutoff 필터를 사용 (12시간 boundary)', async () => {
      ...
      await service.verifyAndRecord('1', UsageKind.RETURN, mockImage(), {})
        .catch(() => undefined)
